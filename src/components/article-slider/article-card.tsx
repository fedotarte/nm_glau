  ...
  if (article.icon) {
    return (
      <span className={className}>
        <Image src={article.icon} alt="" width={size} height={size} />
      </span>
    );
  }
...
type CardVariant = "desktop" | "list";

type ArticleCardFrameProps = {
  article: ArticleConfig;
  variant: CardVariant;
};

const ArticleCardFrame = ({ article, variant }: ArticleCardFrameProps) => {
  ...
  const content =
    variant === "desktop" ? (
      <DesktopCardContent article={article} isAuthenticated={isAuthenticated} />
    ) : (
      <ListCardContent article={article} />
    );

  if (isInDev) {
    const disabledClassName =
      variant === "desktop" ? styles.cardDisabled : styles.mobileCardDisabled;

    return <div className={disabledClassName}>{content}</div>;
  }
  ...
  return (
    <Link href={articlePath} className={className}>
      {content}
    </Link>
  );
};
...
export const DesktopArticleCard = ({ article }: { article: ArticleConfig }) => (
  <ArticleCardFrame article={article} variant="desktop" />
);
